...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            ...
            y_left_pos = np.sum(y_left)
            y_left_neg = len(y_left) - y_left_pos
            y_right_pos = np.sum(y_right)
            y_right_neg = len(y_right) - y_right_pos

            gini_left = 1 - (y_left_pos / len(y_left))**2 - (y_left_neg / len(y_left))**2
            gini_right = 1 - (y_right_pos / len(y_right))**2 - (y_right_neg / len(y_right))**2

            gini_gain = 1 - (len(y_left) / (len(y_left)+len(y_right)) * gini_left) - (len(y_right) / (len(y_left) + len(y_right)) * gini_right)

            # Aktualizacja najlepszego zysku
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is not None:
            # Wybieramy losowo rodzaje cech brane pod uwage, ich ilosc zalezy od parametru feature_subset
            features = np.random.choice(X.shape[1], feature_subset, replace=False)
        else:
            # Jesli uzywamy DecisionTree to bierzemy pod uwage wszystkie rodzaje cech
